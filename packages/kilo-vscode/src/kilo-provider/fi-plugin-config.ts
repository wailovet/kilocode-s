...
async function readFile(): Promise<{ enabled: boolean }> {
  try {
...
async function writeFile(enabled: boolean): Promise<void> {
...
export async function handleFiMessage(
  ...
  if (message.type === "fiGetConfig") {
    const { enabled } = await readFile()
    ctx.post({ type: "fiConfig", enabled })
    return true
  }
  if (message.type === "fiSetConfig" && typeof message.enabled === "boolean") {
    await writeFile(message.enabled)
